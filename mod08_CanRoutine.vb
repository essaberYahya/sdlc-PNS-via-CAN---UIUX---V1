...
Module mod08_CanRoutine
    ...
    Public rpCanDevice As New XLDriver
    Public foo As New TPGM_USER()

    Public Function fCanPortOpen(ByRef rCanPortHandle As Integer, ByVal rCanPort As ULong, ByVal rCanSpeed As UInteger) As Integer
        ...
        Try
            ...
            rXlStatus = rpCanDevice.XL_OpenDriver()
            If rXlStatus <> 0 Then
                MsgBox("XL_OpenDriver: FAIL. Error code: " & rXlStatus.ToString & " [" & rXlStatus & "]")
                Return (1)
            End If

            rXlStatus = rpCanDevice.XL_OpenPort(rCanPortHandle, "xlCANcontrol", rCanPort, rCanPort, 256, XL_BUS_TYPE_CAN)
            If rXlStatus <> 0 Then
                MsgBox("XL_OpenPort: FAIL. Error code: " & rXlStatus.ToString & " [" & rXlStatus & "]")
                Return (1)
            End If

            rXlStatus = rpCanDevice.XL_CanSetChannelBitrate(rCanPortHandle, rCanPort, rCanSpeed)
            If rXlStatus <> 0 Then
                MsgBox("XL_CanSetChannelBitrate: FAIL. Error code: " & rXlStatus.ToString & " [" & rXlStatus & "]")
                Return (1)
            End If

            rXlStatus = rpCanDevice.XL_ActivateChannel(rCanPortHandle, rCanPort, XL_BUS_TYPE_CAN, XL_ACTIVATE_RESET_CLOCK)
            If rXlStatus <> 0 Then
            ...
            'rpMsgLogClass.MsgPrintLogMultyBay("@FG{Purple}DBG_C_1.fCanPortOpen_Port", rpRpkBayInUse, CStr(rCanPort), True)
            'rpMsgLogClass.MsgPrintLogMultyBay("@FG{Purple}DBG_C_1.fCanPortOpen_Speed", rpRpkBayInUse, CStr(rCanPort), True)
    ...
    Public Function fCanPortClose(ByVal rCanPortHandle As Integer, ByVal rCanPort As ULong) As Integer

        Dim rXlStatus As XLClass.XLstatus

        Try

            rXlStatus = rpCanDevice.XL_DeactivateChannel(rCanPortHandle, rCanPort)
            If rXlStatus <> 0 Then
                MsgBox("XL_DeactivateChannel: FAIL. Error code: " & rXlStatus.ToString & " [" & rXlStatus & "]")
                Return (1)
            End If
            rXlStatus = rpCanDevice.XL_ClosePort(rCanPortHandle)
            If rXlStatus <> 0 Then
                MsgBox("XL_ClosePort: FAIL. Error code: " & rXlStatus.ToString & " [" & rXlStatus & "]")
                Return (1)
            End If
            rXlStatus = rpCanDevice.XL_CloseDriver
            If rXlStatus <> 0 Then
            ...
            'rpMsgLogClass.MsgPrintLogMultyBay("@FG{Purple}DBG_C_1.0_fCanPortClose_Port", rpRpkBayInUse, CStr(rCanPort), True)
        ...
        'If rpIniPrintTraceOnFile Then sFileTxtAppend("---------------------------------------------------------------------------------") ' *** End of Can Trace file

    End Function

    Public Function fCanRxFrames(ByVal rCanPort As ULong, ByVal rRxID As String, _
                                 ByRef rFrameRx() As String, Optional ByRef rTimeStamp As ULong = 0, _
        ...
        If rRxId_Dec > &HFFF Then
            rpCanDevice.XL_CanResetAcceptance(rpCanPortHandle, rCanPort, XL_CAN_EXT)
        Else
            rpCanDevice.XL_CanResetAcceptance(rpCanPortHandle, rCanPort, XL_CAN_STD)
        End If

        ' *** Ricevo Dato aggiornato
        Dim rMaxTry As Integer = 0
        Do
            rXlStatus = rpCanDevice.XL_Receive(rpCanPortHandle, rXlEventRx)
        ...
        If rRxId_Dec = rId Then
            ...
            'If rRxData <> " " And rpIniPrintTraceOnFile Then sFileTxtAppend(CStr(Date.Today) & " - TestID:" & CStr(rpNtest) & " - [RX]-[ID: &H" & fConv_DecToHex(rRxId_Dec) & "]-[DATA: &H" & rRxData & "]") 'scrivo il frame ricevuto sul file di trace
    ...
    Public Function fCanTxFrames(ByVal rCanPort As ULong, ByVal rTxID As String, ByRef rFrameTx As String) As Long
        ...
        'rpMsgLogClass.MsgPrintLogMultyBay("@FG{Purple}DBG_C_1.0_fCanTxFrames_CAN_PORT", rpRpkBayInUse, rCanPort.ToString, True)
        ...
        Try
            ...
            If rFrameTx(0) <> "" Then
                ...
                ' *** Filtro di ricezione
                rpCanDevice.XL_CanAddAcceptanceRange(rpCanPortHandle, rCanPort, rId, rId)
                ...
                ' *** Send data
                rXlStatus = rpCanDevice.XL_CanTransmit(rpCanPortHandle, rCanPort, rXlEventTx)
                If rXlStatus <> 0 Then
                ...
                'If rpIniPrintTraceOnFile Then sFileTxtAppend(CStr(Date.Today) & " - TestID:" & CStr(rpNtest) & " - [TX]-[ID: &H" & rTxID & "]-[DATA: &H" & rFrameTx & "]") ' *** scrivo il frame trasmesso sul file di trace
    ...
    Public Function fCanBufferClear(Optional ByVal rWaitAfterClear As Integer = 1, Optional ByVal rMsgToBePreserved As Integer = 5) As Long
        ...
        rpCanDevice.XL_GetReceiveQueueLevel(rpCanPortHandle, rMsgNumber)
    ...
    Public Function fCanBufferClearFlush(Optional ByVal rWaitAfterClear As Integer = 1, Optional ByVal rMsgToBePreserved As Integer = 5) As Long
